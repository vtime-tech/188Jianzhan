...
if ($userrow['uid']!=1 && $userrow['active']!=9){exit("<script language='javascript'>alert('无权限!');window.location.href='index.php';</script>");}
...
if ($userrow['uid'] == 1){
	$users=$db->rs("SELECT * FROM kaxiao_admin WHERE active = 1 or active = 0 ORDER BY uid LIMIT $start,$pagesize");
	$pages=ceil($db->count('SELECT count(*) FROM kaxiao_admin WHERE active = 1 or active = 0')/$pagesize);
}else{
    $users=$db->rs("SELECT * FROM kaxiao_admin WHERE adduser='{$userrow['user']}' ORDER BY uid LIMIT $start,$pagesize");
    $pages=ceil($db->count("SELECT count(*) FROM kaxiao_admin adduser='{$userrow['user']}'")/$pagesize);
}
...
if ($_GET['deban']){
    ...
    if ($id == 1){
        ...
        if($db->query("UPDATE kaxiao_admin SET active=1 WHERE uid='{$id}'")){
...
if ($_GET['peie']){
  if ($userrow['uid'] != 1 && $userrow['limit'] <= 0){
    ...
    if ($userrow['uid'] != 1 && $value > $userrow['limit']){
        exit("<script language='javascript'>alert('配额不足!');window.location.href='userlist.php';</script>");
    }	
    if ($id == 1){
        exit("<script language='javascript'>alert('无效操作!');window.location.href='userlist.php';</script>");
    }else{
        $rowlimit = $db->get_row("SELECT `limit` FROM kaxiao_admin WHERE uid = '{$id}'");
        $values = $value + $rowlimit['limit'];
        if($db->query("UPDATE `kaxiao_admin` SET `limit` = '{$values}' WHERE uid='{$id}'")){
  if ($userrow['uid'] != 1){
        $userrow['limit'] = $userrow['limit'] - $value;
...
if($_GET['update']){
    $id = $_GET['update'];
    if($userrow['uid'] != 1){
        exit("<script language='javascript'>alert('无权限!');window.location.href='userlist.php';</script>");
    }else{
...
<?php if ($users){foreach($users as $user){ ?>
 <tr class="demo">
            ...
            <td><?php
                        ...
                        <td><a href="#" onclick="disp_prompt(<?=$user['uid']?>,0)"><?=$user['limit']?>/添加</a>
          </td>
           <td><?php if ($user['active'] == 1){ ?>
                                    ...
                                    <a class="btn btn-primary" href="?update=<?=$user['uid']?>">升级</a>
                                    <?php } ?>
